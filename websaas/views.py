...
def get_scan_res_json(task_id):
    ...
    print(json.dumps(statistical_one))
    return [res_list, keyword_res_list, context]
...
def get_report_html(request):
    ...
    task_id = request.GET.get("task_id")
    task_id = str(task_id)
    ip_scan, web_scan, context = get_scan_res_json(task_id)
    if context['data']:

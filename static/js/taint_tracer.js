...
var _tokenTaintTracer = _tokenTaintTracer || {
    ...
    log_data_flow_sink: function (taint, frame_data) {
        console.log("log_data_flow_sink", taint);
        _tokenTaintTracer.data_flow_sinks[taint] =
            _tokenTaintTracer.data_flow_sinks[taint] || [];
    ...
    find_and_log_taint: function (func, arguments, object_name, function_name) {
        ...
        if (object_name==="jQuery"&&( function_name === "ajax"|| function_name === "get" ||function_name === "post" ))
        {
            console.log("Chrome ");
            console.log(arguments);
            console.log(function_name);
        }
        if (object_name==="XMLHttpRequest.prototype")
        {
            console.log(object_name)
        }
        // for (var taint in _tokenTaintTracer.taints)
        // {
        //     if (!_tokenTaintTracer.taints.hasOwnProperty(taint) || (_tokenTaintTracer.taints[taint].stop_at_first && _tokenTaintTracer.data_flow_sinks[taint]))
        //     {
        //         continue;
        //     }
        //
        ...
        switch (Object.prototype.toString.call(object)) {
            ...
            case '[object Array]':
                for (var i = 0; i < object.length; i++) {
                    tainted = _tokenTaintTracer.find_taint_recursively(taint, object[i], depth);
    ...
    add_trace_to_namespace: function (namespace) {
        for (var name in namespace)
        {
            ...
            const potentialFunction = namespace.name;
...
_tokenTaintTracer.initialize();
_tokenTaintTracer.update_tracers();